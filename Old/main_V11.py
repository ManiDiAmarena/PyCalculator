...
class Calcolatrice:
    def __init__(self, master):
        ...
        master.geometry("400x780") # Altezza aumentata per la nuova riga
        ...
        # Layout aggiornato: Nuova riga 0 per log, ln, x^y, n!
        # Le righe successive sono scalate
        layout_pulsanti = [
            # Riga 0: Funzioni logaritmiche e placeholder per potenza/fattoriale
            ('log', 0, 0, 1, 1, 'funzione_log'), 
            ('ln', 0, 1, 1, 1, 'funzione_log'), 
            ('xʸ', 0, 2, 1, 1, 'funzione_placeholder'), # Placeholder per potenza
            ('n!', 0, 3, 1, 1, 'funzione_placeholder'), # Placeholder per fattoriale
            # Riga 1 (precedente 0): Funzioni trigonometriche inverse e 1/x
            ('sin⁻¹', 1, 0, 1, 1, 'funzione_trig_inv'), 
            ('cos⁻¹', 1, 1, 1, 1, 'funzione_trig_inv'), 
            ('tan⁻¹', 1, 2, 1, 1, 'funzione_trig_inv'), 
            ('1/x', 1, 3, 1, 1, 'funzione_unaria'),
            # Riga 2 (precedente 1): Funzioni trigonometriche e %
            ('sin', 2, 0, 1, 1, 'funzione_trig'), 
            ('cos', 2, 1, 1, 1, 'funzione_trig'), 
            ('tan', 2, 2, 1, 1, 'funzione_trig'), 
            ('%', 2, 3, 1, 1, 'funzione_unaria'),
            # Riga 3 (precedente 2): Costanti e modalità angolare
            ('π', 3, 0, 1, 1, 'costante'), ('e', 3, 1, 1, 1, 'costante'), 
            ('deg', 3, 2, 1, 1, 'modalita_angolo'), ('rad', 3, 3, 1, 1, 'modalita_angolo'),
            # Riga 4 (precedente 3): Parentesi, radice, quadrato
            ('(', 4, 0, 1, 1, 'parentesi'), (')', 4, 1, 1, 1, 'parentesi'), 
            ('√', 4, 2, 1, 1, 'funzione_unaria'), ('x²', 4, 3, 1, 1, 'funzione_unaria'),
            # Riga 5 (precedente 4): Clear, backspace, operatori
            ('C', 5, 0, 1, 1, 'clear'), ('CE', 5, 1, 1, 1, 'clear_entry'), ('←', 5, 2, 1, 1, 'backspace'), ('/', 5, 3, 1, 1, 'operatore'),
            # Riga 6 (precedente 5): Numeri e operatori
            ('7', 6, 0, 1, 1, 'numero'), ('8', 6, 1, 1, 1, 'numero'), ('9', 6, 2, 1, 1, 'numero'), ('*', 6, 3, 1, 1, 'operatore'),
            # Riga 7 (precedente 6): Numeri e operatori
            ('4', 7, 0, 1, 1, 'numero'), ('5', 7, 1, 1, 1, 'numero'), ('6', 7, 2, 1, 1, 'numero'), ('-', 7, 3, 1, 1, 'operatore'),
            # Riga 8 (precedente 7): Numeri e operatori
            ('1', 8, 0, 1, 1, 'numero'), ('2', 8, 1, 1, 1, 'numero'), ('3', 8, 2, 1, 1, 'numero'), ('+', 8, 3, 1, 1, 'operatore'), 
            # Riga 9 (precedente 8): +/- , 0, . 
            ('+/-', 9, 0, 1, 1, 'negate'), ('0', 9, 1, 1, 1, 'numero'), ('.', 9, 2, 1, 1, 'numero'), 
            # Riga 10 (precedente 9): =
            ('=', 10, 0, 4, 1, 'uguale') 
        ]
        ...
        for (testo, riga, col, cspan, rspan, tipo) in layout_pulsanti:
            ...
            # Applica Funzione.TButton a tutti i tipi di funzione, inclusi i log
            elif tipo in ['parentesi', 'funzione_unaria', 'costante', 'funzione_trig', 'funzione_trig_inv', 'funzione_log', 'funzione_placeholder']: 
        ...
        for i in range(11): # 11 righe di pulsanti (da 0 a 10)
    ...
    def gestisci_click(self, testo_pulsante):
        ...
        # --- NUOVA LOGICA PER FUNZIONI LOGARITMICHE ---
            ...
            try:
                ...
                if valore_input <= 0:
                    self.expression_on_display = "Errore: Dominio log"
                    self.clear_display_on_next_digit = True; self.aggiorna_display(); return

                if testo_pulsante == 'log': # Logaritmo base 10
                    risultato_log = math.log10(valore_input)
                elif testo_pulsante == 'ln': # Logaritmo naturale (base e)
                    risultato_log = math.log(valore_input)
            ...
            self.clear_display_on_next_digit = True
        # --- FINE LOGICA FUNZIONI LOGARITMICHE ---
