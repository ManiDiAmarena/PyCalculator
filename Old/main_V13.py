...
class Calcolatrice:
    def __init__(self, master):
        ...
        layout_pulsanti = [
            ...
            ('xʸ', 0, 2, 1, 1, 'operatore_potenza'), # Tipo aggiornato per xʸ
            ('n!', 0, 3, 1, 1, 'funzione_unaria'), 
        ...
        # Stile per funzioni, parentesi, costanti, e ora anche operatore potenza
        ...
        for (testo, riga, col, cspan, rspan, tipo) in layout_pulsanti:
            ...
            # Applica Funzione.TButton a tutti i tipi di funzione, inclusi i log e operatore_potenza
    ...
    def _evaluate_current_expression(self):
        ...
        # Rimuovi operatori finali, inclusi '**'
        while temp_eval and (temp_eval[-1] in "+-*/(" or temp_eval.endswith('**')):
            if temp_eval.endswith('**'):
                temp_eval = temp_eval[:-2]
            else:
    ...
    def gestisci_click(self, testo_pulsante):
        numeri = "0123456789"
        operatori_base = "+-*/" # Escludiamo '**' da qui per gestirlo a parte
        ...
        if testo_pulsante in numeri:
            ...
            for char_idx in range(len(temp_expr) - 1, -1, -1):
                ...
                if char in operatori_base + "()": break # Non considerare '**' qui
            ...
            if open_p > closed_p and self.expression_on_display and \
               self.expression_on_display[-1] not in operatori_base + '(**': # Aggiunto '**'
                self.expression_on_display += ')'; self.clear_display_on_next_digit = False
        
        # --- MODIFICA: Logica per xʸ (come operatore) ---
        elif testo_pulsante == 'xʸ':
            if not self.expression_on_display or \
               self.expression_on_display[-1] in operatori_base + '(**' or \
               self.expression_on_display[-1] == '(':
                return # Non aggiungere se l'ultimo è un operatore, '**', o '('
            self.expression_on_display += '**'
            self.clear_display_on_next_digit = False # Aspetta l'esponente
        # --- FINE MODIFICA xʸ ---
            
        elif testo_pulsante in operatori_base:
            # (Logica operatori base invariata, ma assicurati che non interferisca con '**')
            ...
            # Controlla se gli ultimi due caratteri sono '**'
            if len(self.expression_on_display) >= 2 and self.expression_on_display[-2:] == '**':
                if testo_pulsante == '-': # Permetti esponente negativo
                    self.expression_on_display += testo_pulsante
                # Altrimenti non fare nulla o sostituisci '**' se l'utente cambia idea
                # Per ora, non permettiamo di sostituire '**' con un operatore base direttamente
                self.aggiorna_display(); return


            ultimo_carattere = self.expression_on_display[-1] if self.expression_on_display else None
            if ultimo_carattere in operatori_base : # Esclude '**'
