...
class Calcolatrice:
    def __init__(self, master):
        ...
        layout_pulsanti = [
            ...
            ('xʸ', 0, 2, 1, 1, 'funzione_placeholder'), 
            ('n!', 0, 3, 1, 1, 'funzione_unaria'), # Tipo aggiornato per n!
    ...
    def gestisci_click(self, testo_pulsante):
        ...
        # --- NUOVA LOGICA PER n! (Fattoriale) ---
        elif testo_pulsante == 'n!':
            if not self.expression_on_display or self.expression_on_display[-1] in operatori_base + '(':
                self.aggiorna_display(); return
            try:
                valore_input = self._evaluate_current_expression()
                # Controlla se è un intero non negativo
                if isinstance(valore_input, float) and not valore_input.is_integer():
                    self.expression_on_display = "Errore: Input non intero"
                elif valore_input < 0:
                    self.expression_on_display = "Errore: Dominio n!"
                else:
                    risultato_fattoriale = math.factorial(int(valore_input))
                    self.expression_on_display = self._format_result(risultato_fattoriale)
            except (SyntaxError, ValueError): # ValueError può essere sollevato da math.factorial per non interi
                self.expression_on_display = "Errore: Input non intero"
            except OverflowError: # Se il numero è troppo grande per il fattoriale
                self.expression_on_display = "Errore: Overflow"
            except Exception as e:
                print(f"Errore in n!: {e}"); self.expression_on_display = "Errore"
            self.clear_display_on_next_digit = True
        # --- FINE LOGICA n! ---
