...
class Calcolatrice:
    def __init__(self, master):
        ...
        master.geometry("400x830") # Altezza aumentata per le nuove righe
        ...
        self.angle_mode = "rad" 
        self.memory_value = 0.0 # Variabile per la memoria
        ...
        # Layout aggiornato: Nuove righe per funzioni di memoria
        layout_pulsanti = [
            ...
            ('MS', 0, 2, 1, 1, 'memoria'), 
            ('M+', 0, 3, 1, 1, 'memoria'),
            # Riga 1 (precedente 0): log, ln, x^y, M- (n! sostituito)
            ('log', 1, 0, 1, 1, 'funzione_log'), 
            ('ln', 1, 1, 1, 1, 'funzione_log'), 
            ('xʸ', 1, 2, 1, 1, 'operatore_potenza'), 
            ('M-', 1, 3, 1, 1, 'memoria'), # Sostituito n! con M-
            # Riga 2 (precedente 1): Funzioni trigonometriche inverse e 1/x
            ('sin⁻¹', 2, 0, 1, 1, 'funzione_trig_inv'), 
            ('cos⁻¹', 2, 1, 1, 1, 'funzione_trig_inv'), 
            ('tan⁻¹', 2, 2, 1, 1, 'funzione_trig_inv'), 
            ('1/x', 2, 3, 1, 1, 'funzione_unaria'),
            # Riga 3 (precedente 2): Funzioni trigonometriche e %
            ('sin', 3, 0, 1, 1, 'funzione_trig'), 
            ('cos', 3, 1, 1, 1, 'funzione_trig'), 
            ('tan', 3, 2, 1, 1, 'funzione_trig'), 
            ('%', 3, 3, 1, 1, 'funzione_unaria'),
            # Riga 4 (precedente 3): Costanti e modalità angolare
            ('π', 4, 0, 1, 1, 'costante'), ('e', 4, 1, 1, 1, 'costante'), 
            ('deg', 4, 2, 1, 1, 'modalita_angolo'), ('rad', 4, 3, 1, 1, 'modalita_angolo'),
            # Riga 5 (precedente 4): Parentesi, radice, quadrato
            ('(', 5, 0, 1, 1, 'parentesi'), (')', 5, 1, 1, 1, 'parentesi'), 
            ('√', 5, 2, 1, 1, 'funzione_unaria'), ('x²', 5, 3, 1, 1, 'funzione_unaria'),
            # Riga 6 (precedente 5): Clear, backspace, operatori
            ('C', 6, 0, 1, 1, 'clear'), ('CE', 6, 1, 1, 1, 'clear_entry'), ('←', 6, 2, 1, 1, 'backspace'), ('/', 6, 3, 1, 1, 'operatore'),
            # Riga 7 (precedente 6): Numeri e operatori
            ('7', 7, 0, 1, 1, 'numero'), ('8', 7, 1, 1, 1, 'numero'), ('9', 7, 2, 1, 1, 'numero'), ('*', 7, 3, 1, 1, 'operatore'),
            # Riga 8 (precedente 7): Numeri e operatori
            ('4', 8, 0, 1, 1, 'numero'), ('5', 8, 1, 1, 1, 'numero'), ('6', 8, 2, 1, 1, 'numero'), ('-', 8, 3, 1, 1, 'operatore'),
            # Riga 9 (precedente 8): Numeri e operatori
            ('1', 9, 0, 1, 1, 'numero'), ('2', 9, 1, 1, 1, 'numero'), ('3', 9, 2, 1, 1, 'numero'), ('+', 9, 3, 1, 1, 'operatore'), 
            # Riga 10 (precedente 9): +/- , 0, . 
            ('+/-', 10, 0, 1, 1, 'negate'), ('0', 10, 1, 1, 1, 'numero'), ('.', 10, 2, 1, 1, 'numero'), 
            # Riga 11 (precedente 10): =
            ('=', 11, 0, 4, 1, 'uguale') 
        ]
        ...
        # (Definizioni stili Numero, Operatore, Clear, Uguale, Funzione, Modalita, ModalitaAttiva invariate)
        ...
        # --- NUOVO STILE PER PULSANTI MEMORIA ---
        style.configure("Memoria.TButton", 
                        font=common_font, 
                        padding=common_padding,
                        background='#ADD8E6', # Azzurro chiaro per memoria (LightBlue)
                        foreground='black', 
                        relief=tk.RAISED) 
        style.map("Memoria.TButton",
                  foreground=common_foreground_black,
                  background=[('active', '#ADD8E6'), ('!active', '#ADD8E6')], # Sfondo non cambia
                  relief=common_relief_map_hover_effect)
        # --- FINE NUOVO STILE ---
        ...
        for i in range(12): # 12 righe di pulsanti (da 0 a 11)
    ...
    def gestisci_click(self, testo_pulsante):
        ...
        if "Errore" in self.expression_on_display and testo_pulsante not in ['C', '←']:
            self.aggiorna_display(); return # Permetti solo C o backspace su errore
        ...
        # --- NUOVA LOGICA PER FUNZIONI DI MEMORIA ---
        elif testo_pulsante == 'MC': # Memory Clear
            self.memory_value = 0.0
            # Non cambia il display, solo la memoria interna
        
        elif testo_pulsante == 'MR': # Memory Recall
            self.expression_on_display = self._format_result(self.memory_value)
            self.clear_display_on_next_digit = True # Il valore richiamato è un nuovo input
        
        elif testo_pulsante == 'MS': # Memory Store
            if not self.expression_on_display or "Errore" in self.expression_on_display:
                return # Non memorizzare se il display è vuoto o in errore
            try:
                valore_da_memorizzare = self._evaluate_current_expression()
                self.memory_value = valore_da_memorizzare
                self.clear_display_on_next_digit = True # Dopo MS, il prossimo input è nuovo
            except (SyntaxError, ValueError):
                self.expression_on_display = "Errore Sintassi" # Mostra errore se non si può valutare
            except Exception as e:
                print(f"Errore in MS: {e}"); self.expression_on_display = "Errore"
                
        elif testo_pulsante == 'M+': # Memory Add
            if not self.expression_on_display or "Errore" in self.expression_on_display:
                return
            try:
                ...
                print(f"Errore in M+: {e}"); self.expression_on_display = "Errore"

        elif testo_pulsante == 'M-': # Memory Subtract
            if not self.expression_on_display or "Errore" in self.expression_on_display:
                return
            try:
                ...
                print(f"Errore in M-: {e}"); self.expression_on_display = "Errore"
        # --- FINE LOGICA FUNZIONI DI MEMORIA ---
