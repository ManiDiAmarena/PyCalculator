...
class Calcolatrice:
    def __init__(self, master):
        ...
        layout_pulsanti = [
            ...
            ('%', 0, 2, 1, 1, 'funzione_placeholder'), # % rimane placeholder
            ('x²', 0, 3, 1, 1, 'funzione_unaria'), # Tipo aggiornato per x²
        ...
        for (testo, riga, col, cspan, rspan, tipo) in layout_pulsanti:
            ...
            # Applica lo stile Funzione.TButton a 'parentesi', 'funzione_unaria', e 'funzione_placeholder'
            elif tipo in ['parentesi', 'funzione_placeholder', 'funzione_unaria']: 
    ...
    def gestisci_click(self, testo_pulsante):
        ...
        # --- NUOVA LOGICA PER x² ---
            ...
            if not self.expression_on_display or self.expression_on_display[-1] in operatori_base + '(':
                # Non si può fare il quadrato di un operatore o di una parentesi aperta
                self.aggiorna_display() # Non cambia nulla o potresti mostrare un breve errore
                return

            if self.expression_on_display[-1] == ')':
                # Trova la parentesi aperta corrispondente
                open_p_count = 0
                start_sub_expr = -1
                for i in range(len(self.expression_on_display) - 1, -1, -1):
                    if self.expression_on_display[i] == ')':
                        open_p_count += 1
                    elif self.expression_on_display[i] == '(':
                        open_p_count -= 1
                        if open_p_count == 0:
                            start_sub_expr = i
                            break
                if start_sub_expr != -1:
                    # Inserisci 'pow(' prima della sub-espressione e ',2)' dopo
                    # Esempio: "10+(2+3)" -> "10+pow((2+3),2)"
                    # Questo è più complesso da inserire direttamente nella stringa se si vuole
                    # che eval() lo capisca senza math.pow. Usiamo '**2'.
                    # self.expression_on_display = self.expression_on_display[:start_sub_expr] + \
                    #                              f"pow({self.expression_on_display[start_sub_expr:]},2)"
                    # Più semplice: aggiungi **(2) dopo la parentesi chiusa
                    self.expression_on_display += '**2' # o '**2.0' per forzare float
                else: # Parentesi non bilanciate, errore (anche se la logica di ')' dovrebbe prevenirlo)
                    self.expression_on_display = "Errore Sintassi"
            else: # L'ultimo è un numero
                # Estrai l'ultimo numero
                start_last_num = 0
                for i in range(len(self.expression_on_display) - 1, -1, -1):
                    if self.expression_on_display[i] in operatori_base + '(':
                        start_last_num = i + 1
                        break
                # Non è necessario estrarre, basta aggiungere **2
                # last_num_str = self.expression_on_display[start_last_num:]
                # prefix = self.expression_on_display[:start_last_num]
                # self.expression_on_display = prefix + f"({last_num_str})**2" # Parentesi per sicurezza
                self.expression_on_display += '**2'


            self.clear_display_on_next_digit = True # Dopo x², il risultato è "finale" per quel termine
        # --- FINE LOGICA x² ---
